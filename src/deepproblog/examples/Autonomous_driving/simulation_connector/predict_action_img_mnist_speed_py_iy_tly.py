...
class AD_Eval_Image(Dataset):
    def __init__(self, image, mnist_idx, speed, traffic_light_y, player_car_y, eval_name, env, transform=None):
        ...
        self.speed = speed
        self.traffic_light_y = traffic_light_y
    ...
    def __getitem__(self, idx):
        ...
        speed = self.speed
        tl_pos = self.traffic_light_y
        player_car_y = self.player_car_y
        return image, mnist, speed, player_car_y, INTER_1_END, tl_pos, label

    def to_query(self, i):
        return Query(
            Term("action",
                 Term("tensor", Term(self.eval_name, Term("a"))),
                 Term("tensor", Term("MNIST", Term("b"))),
                 Constant(float(self.speed)),
                 Constant(float(self.player_car_y)),
                 Constant(float(INTER_1_END)),
                 Constant(float(self.traffic_light_y)),
                 Constant(0)),
...
def predict_action_img_mnist_speed_py_iy_tly(img, mnist_idx, speed, traffic_light_y, player_car_y,  model, env):
    datasets = {
        "eval_image": AD_Eval_Image(img, mnist_idx, speed, traffic_light_y, player_car_y, "eval_image", int(env[-1])),  # test transforms are applied
    }
