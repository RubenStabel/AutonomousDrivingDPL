...
name = "autonomous_driving_NeSy_3_{}_{}_{}".format(data_size, env, N)
...
net_x_rel = Network(network_1, "perc_net_version_2_NeSy_x_rel", batching=True)
net_x_rel.optimizer = torch.optim.Adam(network_1.parameters(), lr=1e-3)

network_2 = AD_V2_NeSy_2_net_y_rel()
net_y_rel = Network(network_2, "perc_net_version_2_NeSy_y_rel", batching=True)
net_y_rel.optimizer = torch.optim.Adam(network_2.parameters(), lr=1e-3)

print("###############    LOADING MODEL    ###############")
model = Model("../models/autonomous_driving_NeSy_3.pl", [net_x_rel, net_y_rel])
