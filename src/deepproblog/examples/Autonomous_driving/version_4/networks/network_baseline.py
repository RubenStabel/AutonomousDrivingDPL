...
class AD_V4_baseline_net_0(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, img, mnist, spd, ped_y, inter_y):
        mnist = self.mnist(mnist)
        img = self.image_features(img)
        num = self.numeric_features(torch.cat((spd, ped_y, inter_y), -1))
        z = torch.cat((img, mnist, num), -1),
