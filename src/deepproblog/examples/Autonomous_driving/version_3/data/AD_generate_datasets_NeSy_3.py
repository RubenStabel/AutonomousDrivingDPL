...
def image_file_to_player_car_y(image_data_path: str, df):
    ...
    pc_y = df[(df['iteration'] == int(iter_image)) & (df['image_frame'] == int(frame))]['player_car_y'].values[0]
    return pc_y
...
class AD_Dataset(Dataset):
    def __init__(self, image_paths, classes, dataset_name, transform=None):
        ...
        if transform is None:
            self.transform_img = transforms.Compose([
                transforms.ToTensor(),
                transforms.Resize((64, 64), antialias=False, interpolation=InterpolationMode.NEAREST_EXACT),
    ...
    def __getitem__(self, idx):
        ...
        pc_y = self._get_player_car_y(idx)
        return image, speed, tl_pos, pc_y, MNIST_img, label
    ...
    def to_query(self, i):
        image, speed, tl_pos, pc_y, MNIST_img_idx, label = self.__getitem__(i)
        return Query(
            Term("action",
