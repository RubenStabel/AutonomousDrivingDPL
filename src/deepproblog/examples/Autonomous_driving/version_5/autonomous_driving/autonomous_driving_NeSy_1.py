...
from deepproblog.examples.Autonomous_driving.version_5.data.AD_generate_datasets_NeSy_1 import get_dataset, MNIST_train
...
net_ped.optimizer = torch.optim.Adam(network_1.parameters(), lr=0.0001)
...
net_spd.optimizer = torch.optim.Adam(network_2.parameters(), lr=0.0001)
...
net_tl.optimizer = torch.optim.Adam(network_3.parameters(), lr=0.0001)
...
net_danger_inter.optimizer = torch.optim.Adam(network_5.parameters(), lr=0.0001)

network_6 = AD_V5_NeSy_0_net_traffic_sign()
if pretrain:
    network_6.load_state_dict(torch.load("../pretrain/perc_net_version_5_NeSy_traffic_sign_2.pth"))
net_ts = Network(network_6, "perc_net_version_5_NeSy_traffic_sign", batching=True)
net_ts.optimizer = torch.optim.Adam(network_6.parameters(), lr=0.0001)

print("###############    LOADING MODEL    ###############")
model = Model("../models/autonomous_driving_NeSy_1.pl", [net_ped, net_spd, net_tl, net_danger_inter, net_ts])
