...
class AD_V5_NeSy_0_net_danger_pedestrian(nn.Module):
    def __init__(self):
        super().__init__()
...
class AD_V5_NeSy_0_net_traffic_light(nn.Module):
    def __init__(self, n=3, with_softmax=True, size=16 * 13 * 13):
        super(AD_V5_NeSy_0_net_traffic_light, self).__init__()
        self.with_softmax = with_softmax
        self.size = size
        if with_softmax:
            if n == 1:
...
class AD_V5_NeSy_0_net_danger_distance(nn.Module):
    def __init__(self):
        super().__init__()
        self.image_features = nn.Sequential(
            nn.Conv2d(3, 6, 5),
...
class AD_V5_NeSy_0_net_danger_intersection(nn.Module):
    def __init__(self):
        super().__init__()
        self.image_features = nn.Sequential(
            nn.Conv2d(3, 6, 5),
    ...
    def forward(self, img, pos):
        img = self.image_features(img)
        # pos = self.numeric_features(pos)
        z = torch.cat((img, pos), -1)
        z = self.combined_features(z)
        return z


class AD_V5_NeSy_0_net_traffic_sign(nn.Module):
    def __init__(self, n=3, with_softmax=True, size=16 * 13 * 13):
        super(AD_V5_NeSy_0_net_traffic_sign, self).__init__()
        self.with_softmax = with_softmax
        self.size = size
        if with_softmax:
            if n == 1:
