...
from deepproblog.examples.Autonomous_driving.version_5.pretrain.AD_generate_datasets_NeSy_pretrain import AD_Dataset, get_paths
    ...
    for nn_name in NN_NAME:
        ...
        match nn_name:
            case 'perc_net_version_5_NeSy_danger_pedestrian':
                model = AD_V5_NeSy_0_net_danger_pedestrian()
                num_classes = 4
            case 'perc_net_version_5_NeSy_speed_zone':
                model = AD_V5_NeSy_0_net_speed_zone()
                num_classes = 8
            case 'perc_net_version_5_NeSy_traffic_light':
                model = AD_V5_NeSy_0_net_traffic_light()
                num_classes = 3
            case 'perc_net_version_5_NeSy_intersection':
                model = AD_V5_NeSy_0_net_danger_intersection()
                num_classes = 2
            case 'perc_net_version_5_NeSy_traffic_sign':
                model = AD_V5_NeSy_0_net_traffic_sign()
                num_classes = 3
            case _:
                model = None
                num_classes = 0
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                if nn_name == 'perc_net_version_5_NeSy_danger_pedestrian':
                    ...
                    for j in [0.0, 1.0]:
                        label = F.one_hot(torch.tensor([labels[int(j)].item()]), num_classes=num_classes)[0]
                        label = label.to(torch.float32)
                        pos = torch.tensor([j], dtype=torch.float32)
                        outputs = model(img[0], pos)
                        loss = criterion(outputs, label)
                        loss.backward()
                        optimizer.step()
                        running_loss += loss.item()
